...
interface BatchDeletePatientModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function BatchDeletePatientModal({
  isOpen,
  onClose,
  onDelete,
  isDeleting,
  count
}: BatchDeletePatientModalProps) {
  const [confirmPhrase, setConfirmPhrase] = useState('');

  const targetPhrase = `Xóa ${count} bệnh nhân`;
  ...
  const isConfirmed = confirmPhrase.trim().toLowerCase() === targetPhrase.toLowerCase();
        ...
        <div className="px-6 md:px-8 pb-6 space-y-6 overflow-y-auto custom-scrollbar flex-1 text-left bg-white dark:bg-slate-900">
          <div className="pt-6 space-y-4">
            <p className="text-[15px] font-medium text-slate-600 dark:text-slate-400 leading-relaxed italic-none">
              Hành động này sẽ xóa vĩnh viễn <span className="font-bold text-red-500">{count} hồ sơ</span> bệnh nhân đã chọn và tất cả dữ liệu liên quan (Chỉ số sức khỏe, Lịch hẹn, Đơn thuốc).
            </p>
          </div>
          ...
          <div className="p-4 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 rounded-xl flex items-center gap-3 font-display">
            ...
            <p className="text-[14px] font-bold text-red-600 dark:text-red-400 italic-none mb-0">
              Việc xóa {count} hồ sơ này không thể hoàn tác.
            </p>
          </div>
        </div>
